...
} from '../../src/components/browser/PullRequestBrowserItem';
...
describe('PullRequestBrowserItem', () => {
  ...
  describe('#render()', () => {
    ...
    it('should not load list item if failed api request', () => {
      ...
      expect(component.find('.jp-PullRequestBrowserItemListItem')).toHaveLength(
        0
      );
    });
    ...
    it('should load list item files if nonempty api response and expanded', () => {
      ...
      _data[0].files = [];
      _data[0].files.push(
        new PullRequestFileModel('test.ipynb', 'modified', 12, 23, _data[0])
      );
      component.setState({ data: _data });
      expect(component.find('.jp-PullRequestBrowserItemFileList')).toHaveLength(
        1
      );
    });
