...
} from '../../src/components/diff/PullRequestCommentThread';
import {
  SamplePullRequestCommentThreadItemNew,
  SamplePullRequestCommentThreadItemReply
} from './testutils';
...
describe('PullRequestBrowserItem', () => {
  ...
  describe('#render()', () => {
    ...
    const replyComponent = shallow(
      <PullRequestCommentThread {...replyProps} />
    );
    ...
    it('should show inputs for both components', () => {
      expect(newComponent.find('.jp-PullRequestInputContainer')).toHaveLength(
        1
      );
      expect(replyComponent.find('.jp-PullRequestInputContainer')).toHaveLength(
        1
      );
    });
    it('should show headers for both components', () => {
      expect(newComponent.find('.jp-PullRequestCommentHeader')).toHaveLength(1);
      expect(replyComponent.find('.jp-PullRequestCommentHeader')).toHaveLength(
        1
      );
    });
    it('should init with input if new, init without input if reply', () => {
      expect(
        newComponent.find('.jp-PullRequestInputFormTextArea')
      ).toHaveLength(1);
      expect(
        newComponent.find('.jp-PullRequestInputButtonContainer')
      ).toHaveLength(1);
      expect(newComponent.find('.jp-PullRequestInputFormButton')).toHaveLength(
        0
      );
      expect(
        replyComponent.find('.jp-PullRequestInputFormTextArea')
      ).toHaveLength(0);
      expect(
        replyComponent.find('.jp-PullRequestInputButtonContainer')
      ).toHaveLength(0);
      expect(
        replyComponent.find('.jp-PullRequestInputFormButton')
      ).toHaveLength(1);
    });
